...
export default function ManagementPage() {
  ...
  async function loadAll() {
    ...
    if (assignedIds.length > 0) {
      const { data: suppData } = await supabase.from('suppliers').select('*, factories(name)').in('factory_id', assignedIds).order('name')
  ...
  async function handleAddSupplier(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('suppliers').insert({ factory_id: supplierFactoryId, name: supplierName.trim() })
    if (error) { setSupplierError(error.message); setSupplierSaving(false); return }

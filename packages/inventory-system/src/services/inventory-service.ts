...
import type { PermissionOverride } from '../types/inventory'

export async function loadProfile(userId: string) {
  const db = getServerDb()
  const { data, error } = await db
    .from('profiles')
    .select('*, profile_factories(factory:factories(*))')
    .eq('id', userId)
    .single()
  if (error || !data) return { profile: null, error: error?.message }

  const { profile_factories: factoryLinks, ...profileRow } = data as {
    profile_factories?: { factory: unknown }[]
    [key: string]: unknown
  }
  const factories = factoryLinks?.map((link) => link.factory).filter(Boolean) ?? []
  return { profile: { ...profileRow, factories }, error: null }
}
...
export async function getOwnerDashboardBundle(factoryIds: string[]) {
  const [dash, loaded, remaining] = await Promise.all([
    getOwnerDashboard(factoryIds),
    fetchLoadedDrilldown(factoryIds),
    fetchRemainingDrilldown(factoryIds),
  ])
  return { ...dash, loadedDrilldown: loaded, remainingDrilldown: remaining }
}

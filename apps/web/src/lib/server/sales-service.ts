...
export async function getCurrentSalesUser(): Promise<{
  ...
  const { data: authData, error: authError } = await getServerDb().auth.getUser()
  const auth = authData.user
  if (authError || !auth?.id) return { user: { id: '', email: null }, membership: null, org: null }

  const { data: members } = await getServerDb()
    .from('sales_users')
...
export async function deleteLead(id: string) {
  const { error } = await getServerDb().from('sales_leads').delete().eq('id', id)
  if (error) throw error
...
export async function nextDocNumber(orgId: string, docType: SalesDocType): Promise<string> {
  const { data, error } = await getServerDb().rpc('sales_next_doc_number', {
    p_org: orgId,
    p_type: docType,
  })
  if (error) throw error
...
export async function deleteDocument(id: string) {
  const { error } = await getServerDb().from('sales_documents').delete().eq('id', id)
  if (error) throw error
...
export async function saveExpense(p: {
  ...
  if (rest.id) {
    ...
    await getServerDb().from('sales_expense_splits').delete().eq('expense_id', expenseRow.id)
  } else {
    const { data, error } = await getServerDb()
      .from('sales_expenses')
      .insert(rest)
  ...
  if (splits.length) {
    ...
    const { error } = await getServerDb().from('sales_expense_splits').insert(rows)
    if (error) throw error
  }
...
export async function deleteExpense(id: string) {
  const { error } = await getServerDb().from('sales_expenses').delete().eq('id', id)
  if (error) throw error
}

export async function settleSplit(splitId: string): Promise<SalesExpenseSplit> {
  const { data, error } = await getServerDb().rpc('sales_settle_split', { p_split_id: splitId })
  if (error) throw error
  return data as SalesExpenseSplit
}

export async function settleAllSplits(expenseId: string): Promise<number> {
  const { data, error } = await getServerDb().rpc('sales_settle_all_splits', { p_expense_id: expenseId })
  if (error) throw error
  return Number(data ?? 0)
}

export async function sendSplitReminder(splitId: string): Promise<SalesExpenseSplit> {
  const { data, error } = await getServerDb().rpc('sales_send_split_reminder', { p_split_id: splitId })
  if (error) throw error
  return data as SalesExpenseSplit
}

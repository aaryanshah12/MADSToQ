...
export default function InputerDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const dash = await inventoryApi.getInputerDashboard(userProfile.id)
      const entries = (dash.stats ?? []) as { entry_date: string; tons_loaded: number }[]
      setStats({
        total: entries.length,
        todayCount: entries.filter(e => e.entry_date === today).length,
        totalTons: entries.reduce((s, e) => s + Number(e.tons_loaded), 0),
      })
      setRecentEntries(dash.entries ?? [])
      const entryFactoryIds = Array.from(new Set((dash.entries ?? []).map((e: any) => e.factory_id).filter(Boolean)))

...
export async function fetchFullStore(): Promise<PMCStore> {
  const [
    ...
    supabase.from('pmc_raw_materials').select('*').order('name'),
    supabase.from('pmc_products').select('*').order('name'),
    supabase.from('pmc_product_materials').select('*').order('sort_order'),
    supabase.from('pmc_references').select('*').order('created_at', { ascending: false }),
    supabase.from('pmc_reference_prices').select('*'),
    supabase.from('pmc_product_params').select('*'),
  ])
...
export async function nextRefNumber(): Promise<string> {
  const { data, error } = await supabase.from('pmc_references').select('ref_number')
  if (error) throw new Error(error.message)
  let max = 0
  for (const r of data ?? []) {
...
export async function upsertRawMaterialDb(input: {
  ...
  const { error } = await supabase.from('pmc_raw_materials').insert({
    user_id: userId,
...
export async function deactivateRawMaterialDb(id: string): Promise<void> {
  const { error } = await supabase.from('pmc_raw_materials').update({ is_active: false }).eq('id', id)
  if (error) throw new Error(error.message)
}
...
export async function setProductMaterialsDb(
  ...
  const { error: insError } = await supabase.from('pmc_product_materials').insert(
    rows.map((row, i) => ({
...
export async function createReferenceDb(
  ...
  if (prices.length > 0) {
    const { error: priceError } = await supabase.from('pmc_reference_prices').insert(
      prices.map((p) => ({
...
export async function upsertProductParamsDb(input: {
  ...
  const { error } = await supabase.from('pmc_product_params').upsert(
    {
...
export async function migrateLocalStorageToSupabaseIfNeeded(): Promise<void> {
  ...
  if (storeHasData(remote)) return

  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (!raw) return
    const local = migrateStore(JSON.parse(raw) as PMCStore)
    if (!storeHasData(local)) return
    await importStoreToSupabase(local)
...
async function importStoreToSupabase(store: PMCStore): Promise<void> {
  ...
  for (const m of store.raw_materials) {
    await supabase.from('pmc_raw_materials').upsert({
      id: m.id,
      user_id: userId,
      name: m.name,
      unit: m.unit,
  ...
  for (const p of store.products) {
    await supabase.from('pmc_products').upsert({
      id: p.id,
      user_id: userId,
      name: p.name,
      code: p.code,
  ...
  for (const m of store.product_materials) {
    await supabase.from('pmc_product_materials').upsert({
      id: m.id,
  ...
  for (const r of store.references) {
    await supabase.from('pmc_references').upsert({
      id: r.id,
      user_id: userId,
  ...
  for (const p of store.reference_prices) {
    await supabase.from('pmc_reference_prices').upsert({
      id: p.id,
  ...
  for (const p of store.product_params) {
    await supabase.from('pmc_product_params').upsert({
      id: p.id,
